function init() {
  d3.json(
    "https://isabelle-sanford.github.io/senate-analysis/jsons/senators.json"
  )
    .then(function (data) {
      ...
      bar(
        bar_x,
        bar_y,
        "# of People each Party Represents",
        bar_colors,
        "IS-bar"
      );
      ...
      let rep_sorted_pops = rep_sorted.map(
        (sen) => parseInt(sen.population) / 2
      );
      let rep_sorted_names = rep_sorted.map(
        (sen) => `${sen.senator} (${sen.state})`
      );

      let dem_sorted = sorted_data.filter((d) => d.party !== "Republican");
      let dem_sorted_pops = dem_sorted.map(
        (sen) => parseInt(sen.population) / 2
      );
      let dem_sorted_names = dem_sorted.map(
        (sen) => `${sen.senator} (${sen.state})`
      );
      ...
      let sex_sorted_data = data.sort(
        (a, b) => parseInt(a.gender) - parseInt(b.gender)
      );
      ...
      let sensexlabels = sex_sorted_data.map(
        (g) => `${g.senator} (${sexObj[g.gender]})`
      );
      ...
      let race_sorted_data = data.sort(
        (a, b) => parseInt(a.race) - parseInt(b.race)
      );
      //console.log(data[0]);
      ...
      let senracelabels = race_sorted_data.map(
        (g) => `${g.senator} (${raceObj[g.race]})`
      );
      ...
      let relig_sorted2 = data.sort((a, b) =>
        relig_color_dict[a.religion] > relig_color_dict[b.religion] ? 1 : -1
      );

      let relig_labels = relig_sorted2.map(
        (d) => `${d.senator} (${d.religion})`
      );
      ...
      nonpartychamber_plot(
        ...
        relig_sorted_colors,
        relig_labels,
        "Religion Demographics - Senate",
        "IS-chamber-sen-relig"
      );

      // @TODO: make an ordering of colors approx by group

      d3.json(
        "https://isabelle-sanford.github.io/senate-analysis/jsons/uspop.json"
      ).then(function (attr) {
        ...
        let pop_sex_colors = getColors(getAllSeats([guys_pop, gals_pop]), [
          "purple",
          "salmon",
        ]);
        ...
        let native_pop = sumList(
          native.map((g) => parseInt(g.POPESTIMATE2019))
        );
        let asian_pop = sumList(asian.map((g) => parseInt(g.POPESTIMATE2019)));
        let pacific_pop = sumList(
          pacific.map((g) => parseInt(g.POPESTIMATE2019))
        );
        let multiple_pop = sumList(
          multiple.map((g) => parseInt(g.POPESTIMATE2019))
        );
        ...
        let race_names = {
          ...
          yellow: "Indigenous/Native American",
          green: "Asian American",
          purple: "Hawaiian or Pacific Islander",
          orange: "Multiple races",
        };
        ...
        nonpartychamber_plot(
          ...
          pop_race_colors,
          pop_race_labels,
          "Race Demographics - US Population",
          "IS-chamber-pop-race"
        );
      ...
      d3.json(
        "https://isabelle-sanford.github.io/senate-analysis/jsons/religion.json"
      ).then(function (relig_data) {
        ...
        let relig_colors1 = getColors(
          getAllSeats(Object.values(religion_obj)),
          Object.values(relig_color_dict2)
        );
        let pop_relig_labels = relig_colors1.map(
          (color) => relig_color_dict3[color]
        );
        ...
        nonpartychamber_plot(
          ...
          relig_colors1,
          pop_relig_labels,
          "US Religion Demographics",
          "IS-chamber-pop-relig"
        );
      });
    })
