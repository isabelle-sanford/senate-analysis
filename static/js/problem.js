function init() {
  ...
  d3.json(
    ...
    // SIMPLE BAR========================================
    rep_total_pop = sendata
      .filter((d) => d.party === "Republican")
      .map((sen) => sen.population);
    dem_total_pop = sendata
      .filter((d) => d.party === "Democratic")
      .map((sen) => sen.population);

    other_total_pop = sendata
      .filter((d) => {
    ...
    let race_sorted_data = sendata.sort(
      (a, b) => parseInt(a.race) - parseInt(b.race)
    );

    let raceKey = {
      1: { color: "#DB222A", race: "white" },
    ...
    let senracelabels = race_sorted_data.map(
      (g) => `${g.senator} (${raceKey[g.race].race})`
    );
    ...
    let sen_relig_labels = relig_sorted_data.map(
  ...
  d3.json(
    ...
    let raceKey2 = [
      { n: 1, color: "#DB222A", race: "White", pop: 0 },
    ...
    let us_race_colors = getColors(
      getAllSeats(raceKey2.map((r) => r.pop)),
      raceKey2.map((r) => r.color)
    );
  ...
  d3.json(
    ...
    for (let i = 1; i < relig_data.length; i++) {
      ...
      if (religion_list.includes(curr_relig.Religion)) {
        religionKey.filter((r) => r.religion === curr_relig.Religion)[0].pop +=
          curr_relig.Percent;
      } else if (other_christian.includes(curr_relig.Religion)) {
        religionKey.filter((r) => r.religion === "Other Christian")[0].pop +=
          curr_relig.Percent;
      } else if (curr_relig.Protestant > 0 && curr_relig.Percent < 99) {
        religionKey.filter((r) => r.religion === "Protestant")[0].pop +=
          curr_relig.Percent;
      } else {
    ...
    let us_relig_colors = getColors(
      getAllSeats(religionKey.map((r) => r.pop)),
      religionKey.map((r) => r.color)
    );
  ...
  bar(
    col_sorted.map((s) => s.state),
    colonies_data.map((s) => s.pop),
    "Free White Male 16-year-olds in the 13 Colonies, 1790",
    "#DB222A",
    "colonies-bar"
  );
